...
public class BixiController implements IBixiController {
    ...
    @Override
    public int getTotalTripsLoaded() {
        return trips.size();
    }

    @Override
    public int getUniqueStationsLoaded() {
        SimpleList<BixiStation> unique = new SimpleList<>();
        for (BixiTrip trip : trips) {
            if (findStationIndex(unique, trip.getStartStationName()) == -1) {
                unique.add(new BixiStation(trip.getStartStationName(), 0));
            }
            if (findStationIndex(unique, trip.getEndStationName()) == -1) {
                unique.add(new BixiStation(trip.getEndStationName(), 0));
            }
        }
        return unique.size();
    }
    ...
    @Override
    public MonthComparison compareMonths(int month1, int month2, int k) {
        MonthComparison.MonthStats first = buildMonthStats(month1, k);
        MonthComparison.MonthStats second = buildMonthStats(month2, k);
        return new MonthComparison(first, second);
    }

    private MonthComparison.MonthStats buildMonthStats(int month, int k) {
        int totalTrips = 0;
        SimpleList<BixiStation> startCounts = new SimpleList<>();
        SimpleList<BixiStation> endCounts = new SimpleList<>();
        ...
        for (BixiTrip trip : trips) {
            ...
            int startIdx = findStationIndex(startCounts, trip.getStartStationName());
            if (startIdx == -1) {
                startCounts.add(new BixiStation(trip.getStartStationName(), 1));
            } else {
                BixiStation s = startCounts.get(startIdx);
                s.setTripCount(s.getTripCount() + 1);
            }

            int endIdx = findStationIndex(endCounts, trip.getEndStationName());
            if (endIdx == -1) {
                endCounts.add(new BixiStation(trip.getEndStationName(), 1));
            } else {
                BixiStation s = endCounts.get(endIdx);
                s.setTripCount(s.getTripCount() + 1);
            }
        ...
        sortStationsByCountDesc(startCounts);
        sortStationsByCountDesc(endCounts);

        SimpleList<BixiStation> topStart = firstKStations(startCounts, k);
        SimpleList<BixiStation> topEnd = firstKStations(endCounts, k);

        RushHour rushHour;
        if (dayCount == 0) {
            rushHour = new RushHour(-1, 0.0);
        } else {
            int bestHour = 0;
            double bestAvg = -1;
            ...
            rushHour = new RushHour(bestHour, bestAvg);
        }

        return new MonthComparison.MonthStats(month, totalTrips, topStart, topEnd, rushHour);
    }
    @Override
